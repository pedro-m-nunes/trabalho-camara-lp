...
public interface EventoRepository extends JpaRepository<Evento, Integer> {
	...
	public List<Evento> findAllByOrderByInicio();
	...
	@Query("SELECT e FROM Evento e INNER JOIN e.inscritos d WHERE d.id = ?1 ORDER BY e.inicio")
	public List<Evento> findAllByDeputadoId(Integer deputadoId);
	
	@Query("SELECT e.id FROM Evento e INNER JOIN e.inscritos d WHERE d.id = ?1")
	public List<Integer> findAllIdsByDeputadoId(Integer deputadoId);
	...
	// cancelar inscrição de deputado
